...
bool CBlock::AddToBlockIndex(const CDiskBlockPos &pos, const uint256& hashProof)
{
    // Check for duplicate
    uint256 hash = GetHash();
...
bool CBlock::AcceptBlock()
{
    ...
    if (!AddToBlockIndex(blockPos, hashProof))
...
// <- GEN
        ...
        if (!block.AddToBlockIndex(blockPos, hashGenesisBlock))
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%s) %s  %08x  %s  mint %7s  tx %lu",
            pindex->nHeight,
