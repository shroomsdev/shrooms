...
int64_t CTransaction::GetValueIn(CCoinsView& inputs) const
{
    if (IsCoinBase())
        return 0;
...
bool CTransaction::CheckInputs(CCoinsView &inputs, enum CheckSig_mode csmode, bool fStrictEncodings, CBlock *pblock) const
{
    if (!IsCoinBase())
    {
        int64_t nValueIn = 0;
...
bool CTransaction::GetCoinAge(CCoinsView& inputs, uint64_t& nCoinAge) const
{
...
bool FindBlockPos(CChainDB &chaindb, CDiskBlockPos &pos, unsigned int nAddSize, unsigned int nHeight, uint64_t nTime)
{
